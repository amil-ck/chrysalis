...
export default class DefaultSelection extends React.Component {
    ...
    checkChoices(choices) {
        ...
        while (JSON.stringify(newChoices) !== JSON.stringify(choices)) {
            choices = newChoices;
            newChoices = choices.filter(x => {
                const slice = choices.slice(0, choices.indexOf(x));
                return slice.some(
                    y => {
                        return this.getChoices(y).some(ySelect => this.getDataForSelect(ySelect).map(dict => dict.id).includes(x))
                    }
                ) || this.getFromId(x).type === TYPE;
            });
        }

        const filteredChoice = originalChoices.filter(x => !newChoices.includes(x));
